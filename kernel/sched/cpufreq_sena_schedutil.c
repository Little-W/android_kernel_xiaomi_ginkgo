 ...
 * changes for sena_schedutil: eng.stk <eng.stk@sapo.pt>
 *
 * Copyright (C) 2020, Marisa Kernel Team
 * based on sena_schedutil
 * changes for sena_schedutil: kmou424 <dx2689282850@gmail.com>
 ...
 * @sg_policy: sena_schedutil policy object to compute the new frequency for.
...
#define NL_RATIO 75
#define DEFAULT_DOWN_RATE_LIMIT_US 0
#define DEFAULT_HISPEED_FREQ 1363000
#define DEFAULT_HISPEED_LOAD 90
#define DEFAULT_PL 1
#define DEFAULT_UP_RATE_LIMIT_US 750
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the sena_schedutil governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the sena_schedutil governor rejects all other frequency scaling requests.
	 *
...
static struct cpufreq_governor sena_schedutil_gov;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SENA_SCHEDUTIL
struct cpufreq_governor *cpufreq_default_governor(void)
{
	return &sena_schedutil_gov;
}
#endif

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&sena_schedutil_gov);
}
