 ...
 * @sg_policy: renoutil policy object to compute the new frequency for.
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the renoutil governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the renoutil governor rejects all other frequency scaling requests.
	 *
...
static struct cpufreq_governor renoutil_gov;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_renoutil
...
{
	return cpufreq_register_governor(&renoutil_gov);
}
